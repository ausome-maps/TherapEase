    ...
    def __call__(self, request):
        ...
        if auth_header and "/users/profile/me/" in request.path:
            ...
            try:
                ...
                print(f"DEBUG: api_settings.SIGNING_KEY={api_settings.SIGNING_KEY}", flush=True)
                print(f"DEBUG: settings.SECRET_KEY={settings.SECRET_KEY}", flush=True)
                print(f"DEBUG: token={token[:50]}...", flush=True)

                from rest_framework_simplejwt.backends import TokenBackend
                backend = TokenBackend(algorithm="HS256", signing_key=api_settings.SIGNING_KEY)
                ...
                try:
                    from rest_framework_simplejwt.backends import TokenBackend
                    backend2 = TokenBackend(algorithm="HS256", signing_key=settings.SECRET_KEY)
                    validated2 = backend2.decode(token)
                    print(f"DEBUG: validate with SECRET_KEY OK: {validated2}", flush=True)
